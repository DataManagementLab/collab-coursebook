...
class JsonHandler:
    ...
    def create_json_topics_structure(course):
        ...
        for topic in course.get_sorted_topic_list():
            i += 1
            structure = content.CourseStructureEntry.objects.get(topic=topic, course=course)
            ...
            if len(structure_index) == 1:
                ...
                if len(content.CourseStructureEntry.objects.filter(course=course,
                                                                   index__startswith=str(structure_index[0]))) > 1:
